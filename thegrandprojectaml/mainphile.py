...
def infuser(message):
    out_text = 'Это бот-бурмалдот для создания питомцев. Начните с помощью /createapet'
    bot.send_message(message.chat.id, out_text, parse_mode='html')
...
def upgradeiqpart4(message, trve, chosenpet):
    ...
    if users[message.from_user.id].pets[chosenpet].scorechecker([message.from_user.id]):
        bot.send_message(message.chat.id, users[message.from_user.id].pets[chosenpet].scorechecker([message.from_user.id]), parse_mode='html', reply_markup=telebot.types.ReplyKeyboardRemove())
...
def getluck2(message, trvnum, chosenpet):
    ...
    if willchecker(users[message.from_user.id].pets[chosenpet].willcount, users[message.from_user.id].pets[chosenpet].will) == nul:
        ...
        trvnum = trvnum
        chosenpet = users[message.from_user.id].pets[chosenpet]
